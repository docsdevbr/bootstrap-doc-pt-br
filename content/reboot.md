...
bundle. Ultimately Bootstrap 5 will continue to see more [CSS variables]({{<
docsref "/customize/css-variables" >}}) added over time, in order to provide
...
more come with [the `.table` class]({{< docsref "/content/tables" >}}).
...
- `<fieldset>`s have no borders, padding, or margin so they can be easily used
...
`.invisible` class]({{< docsref "/utilities/visibility" >}}) instead.
