...
- Navbars and their contents are fluid by default. Change
  ...
  ways.
- Use our [spacing]({{< docsref "/utilities/spacing" >}}) and [flex]({{<
...
This example uses [background]({{< docsref "/utilities/background" >}}) (
`bg-body-tertiary`) and [spacing]({{< docsref "/utilities/spacing" >}}) (
...
Navbar navigation links build on our `.nav` options with their own modifier
...
`justify-content: space-between`. Use additional [flex utilities]({{< docsref "
/utilities/flex" >}}) as needed to adjust this behavior.
...
Use our [position utilities]({{< docsref "/utilities/position" >}}) to place
...
Add `.navbar-nav-scroll` to a `.navbar-nav` (or other navbar sub-component) to
...
the [offcanvas component]({{< docsref "/components/offcanvas" >}}). We extend
